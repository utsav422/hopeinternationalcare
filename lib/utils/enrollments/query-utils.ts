...
export function buildEnrollmentListQuery(options: {
    ...
    // Cast to any to avoid type issues with chained methods after joins
    ...
    if (filters.length > 0) {
        ...
        const filterConditions = filters
            .map(filter => {
                const col =
                    enrollmentColumnMap[
                        filter.id as keyof typeof enrollmentColumnMap
                    ];
    ...
    // Apply sorting
    const sortColumn =
        enrollmentColumnMap[sortBy as keyof typeof enrollmentColumnMap] ||
