...
import { validateRefundData, RefundValidationError } from '@/lib/utils/refunds';
import {
 ...
 */
export async function handleRefundError(
    error: unknown,
    operation: string
    ...
    logger.error(`Unexpected error in refund ${operation}:`, {
        error: String(error),
    });
    return Promise.reject({
        success: false,
 ...
 */
export async function adminRefundList(params: RefundQueryParams): Promise<
    ApiResponse<{
        data: RefundListItem[];
        total: number;
        page: number;
    ...
    try {
        ...
        if (search) {
            ...
            filterConditions.push(
                sql`(${refunds.reason} ILIKE ${searchFilter} OR ${profiles.full_name} ILIKE ${searchFilter} OR ${profiles.email} ILIKE ${searchFilter})`
            );
        }
        ...
        const countQuery = db
            ...
            .from(refunds)
            .leftJoin(profiles, eq(refunds.user_id, profiles.id))
            .where(whereClause);
 ...
 */
export async function adminRefundDetails(
    id: string
): Promise<ApiResponse<RefundWithDetails>> {
    try {
        await requireAdmin();
 ...
 */
export async function adminRefundCreate(
    data: RefundCreateData
): Promise<ApiResponse<RefundBase>> {
    try {
        await requireAdmin();
        ...
        if (!data.payment_id) {
            return {
                success: false,
                error: 'Payment ID is required for a refund',
        ...
        const createdRefund = await db.transaction(async tx => {
            ...
            if (data.amount > refundableAmount) {
                tx.rollback();
                return {
                    error: `Refund amount cannot exceed the refundable amount of ${refundableAmount}`,
                    code: 'VALIDATION_ERROR',
                };
            }
