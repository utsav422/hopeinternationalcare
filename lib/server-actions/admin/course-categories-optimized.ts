...
import { CourseCategoryValidationError } from '../../utils/course-categories/index';
import {
 ...
 */
export async function handleCourseCategoryError(
    error: unknown,
    operation: string
    ...
    if (error instanceof Error) {
        logger.error(`Course Category ${operation} failed:`, {
            error: error.message,
        });
        return Promise.reject({
            success: false,
    ...
    logger.error(`Unexpected error in course category ${operation}:`, {
        error: String(error),
    });
    return Promise.reject({
        success: false,
...
export async function adminCourseCategoryList(
    params: CourseCategoryQueryParams
): Promise<
    ApiResponse<{
...
> {
    try {
        ...
        return await handleCourseCategoryError(error, 'list');
    }
}
 ...
 */
export async function adminCourseCategoryDetails(
    id: string
): Promise<ApiResponse<CourseCategoryWithDetails>> {
    try {
        await requireAdmin();
        ...
        if (!id || id.trim() === '') {
            return {
                success: false,
                error: 'Course category ID is required',
 ...
 */
export async function adminCourseCategoryCreate(
    data: CourseCategoryCreateData
): Promise<ApiResponse<CourseCategoryBase>> {
    try {
        await requireAdmin();
...
export async function adminCourseCategoryUpdate(
    data: CourseCategoryUpdateData
): Promise<ApiResponse<NewCourseCategory>> {
    try {
        await requireAdmin();
...
export async function adminCourseCategoryDelete(
    id: string
 ...
 */
export async function adminCourseCategoryCheckConstraints(
    id: string
): Promise<ApiResponse<CourseCategoryConstraintCheck>> {
    try {
        await requireAdmin();
