 ...
 */
export async function handlePublicCourseError(
    error: unknown,
    operation: string
    ...
    logger.error(`Unexpected error in public course ${operation}:`, {
...
> {
    try {
        ...
        const data = await db
            ...
            .leftJoin(
                nextIntakeDetails,
                eq(courses.id, nextIntakeDetails.course_id)
            )
            .where(
...
export async function getPublicCourseById(
    ...
    try {
        ...
        // Get the course category if category_id exists
        let category = null;
        if (result[0].category_id) {
            const categoryResult = await db
                ...
                .where(eq(courseCategories.id, result[0].category_id));
        ...
        // Get the course intakes
        const intakesResult = await db
            .select()
            .from(intakes)
            .where(eq(intakes.course_id, result[0].id));

        // Map the result to match PublicCourseDetail interface
        const courseData = {
            id: result[0].id,
            title: result[0].title,
            slug: result[0].slug,
            price:
                typeof result[0].price === 'string'
                    ? Number(result[0].price)
                    : result[0].price,
            level: result[0].level,
            duration_type: result[0].duration_type,
            duration_value: result[0].duration_value,
            created_at: result[0].created_at,
            updated_at: result[0].updated_at,
            course_highlights: result[0].courseHighlights,
            course_overview: result[0].courseOverview,
...
export async function getRelatedCourses(
    courseId: string,
    categoryId: string
): Promise<ApiResponse<RelatedCourse[]>> {
    // Input validation
