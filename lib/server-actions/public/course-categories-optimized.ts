...
} from '@/lib/db/schema/course-categories';
import { 
 ...
 */
export async function publicGetAllCategories(): Promise<ApiResponse<PublicCourseCategoryListItem[]>> {
  try {
    const data = await db
      .select({
    ...
    logger.error('Error fetching public course categories:', error);
    return { 
      success: false, 
