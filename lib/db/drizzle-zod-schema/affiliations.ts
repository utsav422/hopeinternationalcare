...
export const ZodAffiliationInsertSchema = createInsertSchema(affiliations, {
    name: schema =>
        schema.min(3, { message: 'Name must be at least 3 characters long' }),
    type: schema => schema.min(1, { message: 'Type is required' }),
    description: schema =>
        schema
...
export const ZodAffiliationSelectSchema = createSelectSchema(affiliations);

export const ZodAdminAffiliationQuerySchema = z
    .object({
...
export type ZodAdminAffiliationQueryType = z.infer<
    typeof ZodAdminAffiliationQuerySchema
>;
