...
export const customerContactReplies = pgTable(
    'customer_contact_replies',
    {
        ...
        contact_request_id: uuid('contact_request_id')
            .notNull()
            .references(() => customerContactRequests.id, {
                onDelete: 'cascade',
            }),
        ...
        email_status: varchar('email_status', { length: 50 })
            .default('sent')
            .notNull(), // 'sent', 'delivered', 'bounced', 'failed'
        ...
        is_batch_reply: varchar('is_batch_reply', { length: 10 })
            .default('false')
