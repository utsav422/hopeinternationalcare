...
import { ZodDeletedUsersQueryType } from '@/lib/db/drizzle-zod-schema';
import {queryKeys} from "@/lib/query-keys";

export const metadata: Metadata = {
    title: 'Deleted Users | Admin Dashboard | Hope International',
    description: 'Manage deleted users, view deletion history, and restore users in the Hope International admin dashboard.',
    robots: {
        index: false,
        follow: false,
    },
...
export default async function DeletedUsersPageRoute(props: {
    searchParams: Promise<ZodDeletedUsersQueryType>;
}) {
    ...
    const pageSize = parseInt(typeof searchParams.pageSize === 'string' ? searchParams.pageSize : '10') || 10;
    const search = typeof searchParams.search === 'string' ? searchParams.search : '';
    const sortBy = typeof searchParams.sortBy === 'string' ? searchParams.sortBy : 'deleted_at';
    const order = typeof searchParams.order === 'string' && ['asc', 'desc'].includes(searchParams.order)
    ...
    const queryParams:ZodDeletedUsersQueryType  = {
        page,
        pageSize,
        search,
        sortBy,
