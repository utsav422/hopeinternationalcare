...
const publicIntakeQueryKeys = {
  ...
  byCourse: (courseId: string) => [...publicIntakeQueryKeys.all, 'course', courseId] as const,
  bySlug: (slug: string) => [...publicIntakeQueryKeys.all, 'slug', slug] as const,
...
export function useGetCourseIntakesBySlug(slug: string) {
  return useSuspenseQuery({
    ...
    queryFn: async () => {
      ...
      if (!result.success) {
        throw new Error(result.error || 'Failed to fetch course intakes');
      }
      return result;
    },
    staleTime: 1000 * 60 * 5,  // 5 minutes
