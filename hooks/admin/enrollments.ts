...
export const useAdminEnrollmentListByStatus = (status: string) => {
  return useSuspenseQuery({
    queryKey: queryKeys.enrollments.list({ status }),
    queryFn: async () => {
      const result = await cachedAdminEnrollmentList({ status });
      if (!result.success) {
        throw new Error(result.error || 'Failed to fetch enrollments by status');
      }
      return result.data;
    },
    staleTime: 1000 * 60 * 5, // 5 minutes
...
export const useAdminEnrollmentListByUserId = (userId: string) => {
  return useSuspenseQuery({
    ...
    queryFn: async () => {
      ...
      if (!result.success) {
        throw new Error(result.error || 'Failed to fetch user enrollments');
      }
      return result.data;
    },
    staleTime: 1000 * 60 * 5, // 5 minutes
...
export const useAdminEnrollmentUpdate = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<EnrollmentCreateData> }) => 
    ...
    onSuccess: async (result) => {
      if (result.success) {
        ...
        await queryClient.invalidateQueries({
          queryKey: queryKeys.enrollments.detail(result.data?.enrollment.id || '')
        });
      }
    },
  });
...
export const useAdminEnrollmentUpdateStatus = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (update: EnrollmentStatusUpdate) => adminEnrollmentUpdateStatus(update),
    onSuccess: async (result) => {
      if (result.success) {
        await queryClient.invalidateQueries({
          queryKey: queryKeys.enrollments.all
...
export const useAdminEnrollmentBulkStatusUpdate = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (updates: EnrollmentBulkStatusUpdate) => adminEnrollmentBulkStatusUpdate(updates),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: queryKeys.enrollments.all
      });
