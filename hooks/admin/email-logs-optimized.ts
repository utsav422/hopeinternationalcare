...
export function useAdminEmailLogConstraintCheck() {
  return useQuery({
    queryKey: [...emailLogQueryKeys.all, 'constraint-check'],
    queryFn: async (id: string) => adminEmailLogCheckConstraints(id),
...
export function useAdminEmailLogUpdateStatus() {
  ...
  return useMutation({
    mutationFn: (data: EmailLogStatusUpdate) => adminEmailLogUpdateStatus(data),
    onSuccess: async (result) => {
      if (result.success && result.data) {
        // Invalidate specific email log queries
        await queryClient.invalidateQueries({
