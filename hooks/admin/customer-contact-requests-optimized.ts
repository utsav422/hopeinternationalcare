...
export function useAdminCustomerContactRequestCreate() {
  ...
  return useMutation({
    mutationFn: (data: CustomerContactRequestCreateData) => adminCustomerContactRequestCreate(data),
    onSuccess: async () => {
...
export function useAdminCustomerContactRequestUpdate() {
  ...
  return useMutation({
    mutationFn: (data: CustomerContactRequestUpdateData) => adminCustomerContactRequestUpdate(data),
    onSuccess: async (result) => {
      if (result.success && result.data) {
        // Invalidate specific customer contact request queries
        await queryClient.invalidateQueries({
          queryKey: customerContactRequestQueryKeys.detail(result.data.id),
        });
        await queryClient.invalidateQueries({
          queryKey: customerContactRequestQueryKeys.all,
        });
...
export function useAdminCustomerContactRequestConstraintCheck() {
  return useQuery({
    queryKey: [...customerContactRequestQueryKeys.all, 'constraint-check'],
    queryFn: async (id: string) => adminCustomerContactRequestCheckConstraints(id),
...
export function useAdminCustomerContactRequestUpdateStatus() {
  ...
  return useMutation({
    mutationFn: (data: CustomerContactRequestStatusUpdate) => adminCustomerContactRequestUpdateStatus(data),
    onSuccess: async (result) => {
      if (result.success && result.data) {
        // Invalidate specific customer contact request queries
        await queryClient.invalidateQueries({
