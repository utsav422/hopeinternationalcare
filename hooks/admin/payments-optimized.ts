...
const paymentQueryKeys = {
    ...
    byStatus: (status: string) =>
        [...paymentQueryKeys.all, 'status', status] as const,
    byUser: (userId: string) =>
        [...paymentQueryKeys.all, 'user', userId] as const,
...
export function useAdminPaymentUpdateStatus() {
    ...
    return useMutation({
        mutationFn: (data: PaymentStatusUpdate) =>
            adminPaymentUpdateStatus(data),
        onSuccess: async result => {
            if (result.success && result.data) {
                // Invalidate specific payment queries
                await queryClient.invalidateQueries({
...
export function useAdminPaymentMetrics() {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            return {
                success: true,
                data: { total: 0, byStatus: {}, revenue: 0 },
            };
        },
