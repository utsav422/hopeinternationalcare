...
export function useAdminCustomerContactReplyCreate() {
  ...
  return useMutation({
    mutationFn: (data: CustomerContactReplyCreateData) => adminCustomerContactReplyCreate(data),
    onSuccess: async () => {
...
export function useAdminCustomerContactReplyUpdate() {
  ...
  return useMutation({
    mutationFn: (data: CustomerContactReplyUpdateData) => adminCustomerContactReplyUpdate(data),
    onSuccess: async (result) => {
      if (result.success && result.data) {
        // Invalidate specific customer contact reply queries
        await queryClient.invalidateQueries({
          queryKey: customerContactReplyQueryKeys.detail(result.data.id),
        });
        await queryClient.invalidateQueries({
          queryKey: customerContactReplyQueryKeys.all,
        });
...
export function useAdminCustomerContactReplyConstraintCheck() {
  return useQuery({
    queryKey: [...customerContactReplyQueryKeys.all, 'constraint-check'],
    queryFn: async (id: string) => adminCustomerContactReplyCheckConstraints(id),
...
export function useAdminCustomerContactReplyUpdateStatus() {
  ...
  return useMutation({
    mutationFn: (data: CustomerContactReplyStatusUpdate) => adminCustomerContactReplyUpdateStatus(data),
    onSuccess: async (result) => {
      if (result.success && result.data) {
        // Invalidate specific customer contact reply queries
        await queryClient.invalidateQueries({
