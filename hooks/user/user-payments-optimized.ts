...
const userPaymentQueryKeys = {
  ...
  list: (page: number, pageSize: number) => [...userPaymentQueryKeys.lists(), page, pageSize] as const,
...
export const useCreatePayment = () => {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: userPaymentQueryKeys.all });
    },
  });
};

// Query operations
export const useGetUserPaymentHistory = (page: number = 1, pageSize: number = 10, userId: string) => {
