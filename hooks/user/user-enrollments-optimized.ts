    ...
    return useMutation({
        ...
        onSuccess: async () => {
            await queryClient.invalidateQueries({
                queryKey: userEnrollmentQueryKeys.all,
            });
        },
    });
};

// Mutation operations for FormData
export const useCreateEnrollmentFromFormData = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: async (formData: FormData) => {
            const result = await createEnrollmentFromFormData(formData);
            if (!result.success) {
                throw new Error(result.error);
            }
            return result.data;
...
export const useGetUserEnrollments = () => {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            // Return only the array data to maintain backward compatibility
...
// Alternative hook that returns both data and total
export const useGetUserEnrollmentsWithTotal = () => {
    return useQuery({
        queryKey: [...userEnrollmentQueryKeys.list(), 'with-total'],
        queryFn: async () => {
            const result = await getUserEnrollments();
            if (!result.success || !result.data) {
                throw new Error(result.error || 'Failed to fetch enrollments');
            }
            return result.data; // Return the full object with data and total
        },
        staleTime: 1000 * 60 * 5, // 5 minutes
        gcTime: 100 * 60 * 60, // 1 hour
    });
};
