  ...
  for (let i = 1; i <= 10; i++) {
    try {
      ...
      if (response.status === 200) {
        ...
        rateLimitedCount++;
        log(`  ⏱️  Request ${i}: Rate limited (${response.status})`, 'yellow');
      } else {
        log(`  ❌ Request ${i}: Unexpected status (${response.status})`, 'red');
      }
    } catch (error) {
...
async function testInputValidation() {
  ...
  for (const testCase of testCases) {
    try {
      ...
      const isExpectedStatus = testCase.expectStatus.includes(response.status);
      
      if (isExpectedStatus) {
        log(`  ✅ ${testCase.name}: Handled correctly (${response.status})`, 'green');
      } else {
        log(`  ❌ ${testCase.name}: Unexpected status (${response.status})`, 'red');
      }
    } catch (error) {
...
async function testAuthentication() {
  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      ...
      log(`  ❌ Admin endpoint not protected (status: ${response.status})`, 'red');
    }
  } catch (error) {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      log(`  ⚠️  Public endpoint returned status: ${response.status}`, 'yellow');
    }
  } catch (error) {
...
async function testSecurityHeaders() {
  ...
  try {
    ...
    const setHeaders = Object.values(securityHeaders).filter(Boolean).length;
...
async function runSecurityTests() {
  ...
  try {
    ...
    log('  4. Consider implementing additional security measures', 'yellow');
